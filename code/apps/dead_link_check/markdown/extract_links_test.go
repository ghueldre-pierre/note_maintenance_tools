...
func TestExtractAllLinksWithinDocument(t *testing.T) {
	expArr, doc := GetMarkdownDocument()
	gotArr := markdown.ExtractAllLinks(doc)
	for i, got := range gotArr {
		if i < len(expArr) {
			t.Logf("\nExpected :\n%s\nGot :\n%s\n", expArr[i], got)
			if expArr[i] != got {
				t.Errorf("\nExpected :\n%s\nGot :\n%s\n", expArr[i], got)
...
func GetMarkdownDocument() ([]string, string) {
	return []string{
